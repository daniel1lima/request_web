...
async function sendSimpleMessageTemplate(customerName, songName, eventName,  ) {
...
router.post("/orderConfirmed", async (req, res) => {
    const {id, amount, billing_details, receipt_url, payment_intent } = req.body.data.object
    ...
    try {
      ...
      res.status(201).json({ message: "Received the information from the webhook!", entry: req.body.data });
    } catch (error) {
