...
export const updateEvent = async (eventId: string, eventData: any) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/events/update?eventId=${eventId}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      "x-api-key": `eab4c259-7ced-4f9c-bd43-d60e244c183e`,
    },
    body: JSON.stringify({
      eventId,
      ...eventData,
...
export const capturePaymentIntent = async (
  ...
  const response = await apiFetch(
    `/stripe/capturePaymentIntent?intentId=${paymentId}&capture=${amount}`,
    {
      method: "POST",
    }
  );
