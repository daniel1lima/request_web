...
const AllEventsPage = () => {
  ...
  return (
    <div className="relative w-screen bg-gray-900 text-white h-screen">
      <header className="bg-gray-900 dark:bg-gray-900 w-full py-1 px-4 flex items-start h-50 fixed top-0 left-0 z-20">
        ...
        <div className="flex items-center w-full">
          ...
          <div className="flex-grow flex ml-20"> {/* Center the logo */}
            <Image
            ...
            {filteredEvents.length > 0 ? (
              filteredEvents.map((event) => (
                <div key={event.eventId} onClick={() => {
                  localStorage.setItem('eventId', event.eventId); // Store the eventId in local storage
                  redirect(`/event?eventId=${event.eventId}`); // Redirect to the event page
                }} className="cursor-pointer"> {/* Added onClick for redirection */}
                  <EventCard
                    image={event.eventImage}
                    title={event.eventName}
                    date={new Date(event.eventDateTime).toLocaleDateString()} // Format date as string
